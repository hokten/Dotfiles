...
function send_notify(){
  ...
  vol_bar="^ib(1)^fg(#bfd8bd)^pa(0;0)^r(${barw}x${barh})^fg(#dde7c7)^pa(${border};${border})^r($(($barw-(2*$border)))x$(($barh-(2*$border))))"
  vol_bar+="^pa($((($barw-50)/2));4)^fn(Material\-Design\-Iconic\-Font-45)^fg(#AA0066)${vol_icon}"
  vol_bar+="^pa($((($barw-402)/2));${baroffset})^fg(#bfd8bd)^r(402x24)^pa($(((($barw-402)/2)+1));$(($baroffset+1)))^fg(#edeec9)^r(400x22)"
  vol_bar+="^pa($(((($barw-402)/2)+1));$(($baroffset+2)))"
  for ((i=1;i<=$cur_vol;i++)); do
    vol_bar+="^fg(${renkler[$i]})^p(1)^r(3x20)"
  done
  echo -e "${vol_bar}^pa($((($barw-55)/2));90)^fg(#00a676)^fn(TeX Gyre Adventor-18:Bold)${cur_vol}%"
}
